...
```{r}
weighted.mean(c(100,85),c(.04,.07))
```
...
Obtain 90%, 95%, and 99% confidence intervals for the Chechen estimate you calculated in the last class. You'll need the difference in means and the standard error, which are provided below. To practice, use _qnorm()_ to obtain the exact critical values.

```{r}

diff_mean <- -0.553459 # difference in attacks after random artillery fire
se <- 0.4165658 # se for the difference in means
...
# As a class: Using regression for standard error / confidence interval
...
1.6 Conceptual question on confidence intervals:
